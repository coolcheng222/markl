...
| 字符                                                         | 含义                                                         |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| [`\`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions#special-backslash) | 转义                                                         |
| [`^`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions#special-caret) | 一般匹配字串开头,有不同含义                                  |
| [`$`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions#special-dollar) | 匹配子串结束,或者换行符前的内容(当多行时)                    |
...
| [`?`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions#special-questionmark) | 1. 匹配前面一个表达式 0 次或者 1 次。等价于 `{0,1}`。<br />2. 如果**紧跟在任何量词 \*、 +、? 或 {} 的后面**，将会使量词变为**非贪婪**（匹配尽量少的字符），和缺省使用的**贪婪模式**（匹配尽可能多的字符）正好相反。例如，对 "123abc" 使用 `/\d+/` 将会匹配 "123"，而使用 `/\d+?/` 则只会匹配到 "1"。<br />3. 还用于先行断言中,后续有说 |
| [`.`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions#special-dot) | 默认匹配除换行符之外的任何单个字符                           |
...
| [`(?:x)`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions#special-non-capturing-parentheses) | 匹配 'x' 但是不记住匹配项。这种括号叫作*非捕获括号*; 主要用于想要用括号但不想影响捕获的情况 |
...
* 返回值: 
  ...
  `result` 是一个 **类数组对象**，它包含以下内容：

  - `0`: 整个匹配的字符串
  - 捕获到的对象
  - `index`: `1`（匹配开始的位置）
  - `input`: `'cdbbdbsbz'`（原始输入字符串）
  - `groups`: `undefined`（没有使用命名捕获组）

省流: 0是整个对象,123是捕获到的123,index是整个的位置

### 2. match/matchAll

match由String调用,和exec一致

matchAll返回一个iterator

### 3. test/search

test由reg调用,返回true/false

search由string调用,返回索引或-1

### 4. replace

replace由string调用,可以用$n表示捕获对象
